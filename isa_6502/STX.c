...
ISA_6502_INSTR(STX)
{
        ...
        case 2:
            switch ( opcode_context->addressing_mode ) {
                case isa_6502_addressing_zeropage:
                    ...
                    break;
                case isa_6502_addressing_zeropage_y_indexed:
                    ADDR8 += opcode_context->registers->Y;
                    break;
                case isa_6502_addressing_absolute:
    ...
    if ( at_stage == isa_6502_instr_stage_end) {        
        membus_write_addr(opcode_context->memory, ADDR16, opcode_context->registers->X);
    }
    return at_stage;
}

ISA_6502_STATIC_INSTR(STX)
{
    uint16_t    ADDR16;
    uint8_t     ADDR8;
    uint8_t     cycle_count;
    ...
    switch ( opcode_context->addressing_mode ) {
        case isa_6502_addressing_zeropage:
            ...
            break;
        case isa_6502_addressing_zeropage_y_indexed:
            ADDR8 = membus_read_addr(opcode_context->memory, opcode_context->registers->PC++) + opcode_context->registers->Y;
            ADDR16 = ADDR8;
            cycle_count = 3;
            break;
        case isa_6502_addressing_absolute:
...
ISA_6502_DISASM(STX)
{
    ...
    return snprintf(buffer, buffer_len, out_fmt, operand1, operand2, opcode_context->registers->X, opcode_context->registers->X);
#else
    return 0;
#endif
}
