...
)
{
    ...
    if ( at_stage == isa_6502_instr_stage_end ) {
        if ( ! registers_SR_get_bit(opcode_context->registers, register_SR_Bit_N) ) opcode_context->registers->PC = ADDR;
    }
    return at_stage;
}

int
__isa_6502_disasm_BPL(
    isa_6502_instr_context_t    *opcode_context,
    char                        *buffer,
    int                         buffer_len
)
{
    ...
    return snprintf(buffer, buffer_len, "BPL %hhd", *((int8_t*)&operand));
#else
    return 0;
#endif
}
